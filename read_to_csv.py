...
def main(com_port, label_name, official):
    ...
    # recording = 0
    # Determine the initial file index based on existing files
    base_folder_path = "csv/official/" if official else "csv/unofficial/"
    label_folder_path = os.path.join(base_folder_path, label_name)
    if not os.path.exists(label_folder_path):
        os.makedirs(label_folder_path)
        
    # Get the highest file index in the folder
    existing_files = os.listdir(label_folder_path)
    highest_index = -1
    for file in existing_files:
        match = re.match(rf"{label_name}_(\d+).csv", file)
        if match:
            index = int(match.group(1))
            if index > highest_index:
                highest_index = index
    recording = highest_index + 1
